...
def parse_args():
    ...
    parser.add_argument('--mot_classes', action='store_true',
                        help='Show MOT Challenge categories (person, car, motorcycle, bicycle)')
    ...
    parser.add_argument('--webcam', type=int, default=None,
                        help='Camera index for webcam input (e.g., 0 for default camera)')
    ...
    while cap.isOpened() and frame_idx < max_frames:
        ...
        if len(boxes) > 0:
            ...
            if detection_mask.sum() > 0:
                ...
                all_reid_features = reid_features[detection_mask] if reid_features is not None else torch.zeros(
                    (detection_mask.sum().item(), 128), device=device)
                ...
                if args.persons_only:
                    ...
                    if persons_mask.sum() > 0:  # If we have any persons
                        ...
                        all_reid_features = all_reid_features[
                            persons_mask] if all_reid_features is not None else torch.zeros(
                            (persons_mask.sum().item(), 128), device=device)
                # Apply MOT classes filter if specified
                    ...
                    if mot_mask.sum() > 0:  # If we have any MOT objects
                        ...
                        all_reid_features = all_reid_features[
                            mot_mask] if all_reid_features is not None else torch.zeros((mot_mask.sum().item(), 128),
                                                                                        device=device)
                # Custom class ids if specified
                    ...
                    if custom_mask.sum() > 0:  # If we have any custom objects
                        ...
                        all_reid_features = all_reid_features[
                            custom_mask] if all_reid_features is not None else torch.zeros(
                            (custom_mask.sum().item(), 128), device=device)
...
def main():
                ...
                if detection_mask.sum() > 0:
                    ...
                    if args.persons_only:
                        ...
                        if persons_mask.sum() > 0:  # If we have any persons
                            ...
                            all_reid_features = all_reid_features[
                                ...
                                (persons_mask.sum().item(), 128), device=device)
                            print(
                                f"Filtered to show only {persons_mask.sum().item()} person detections out of {len(persons_mask)} total")
                        else:
                    ...
                    # Apply MOT classes filter if specified
                        ...
                        if mot_mask.sum() > 0:  # If we have any MOT objects
                            ...
                            all_labels = all_labels[mot_mask]
                            all_reid_features = all_reid_features[
                                mot_mask] if all_reid_features is not None else torch.zeros(
                                (mot_mask.sum().item(), 128), device=device)
                    ...
                    # Custom class ids if specified
                        ...
                        if custom_mask.sum() > 0:  # If we have any custom objects
                            ...
                            all_reid_features = all_reid_features[
                                ...
                                (custom_mask.sum().item(), 128), device=device)
                            print(
                                f"Filtered to show {custom_mask.sum().item()} objects with class IDs: {args.class_ids}")
                        else:
                    ...
                    if frame_idx == 0 or len(active_tracks) == 0:
                        ...
                        print(
                            f"Frame {frame_idx}: Matched {len(assignments)} tracks, {len(unmatched_tracks)} lost, {len(unmatched_detections)} new")
                    ...
                    for track in current_frame_tracks:
                        ...
                        results_txt.append(
                            f"{frame_idx + 1},{track_id},{x1:.2f},{y1:.2f},{w:.2f},{h:.2f},{score:.4f},-1,-1,-1")
    ...
    # Save tracking results to MOT format file
    output_file = os.path.join(f"{args.seq_name}.txt")
    with open(output_file, "w") as f:
        for line in results_txt:
            f.write(line + "\n")

    print(f"Saved MOT-format tracking results to: {output_file}")
